...
state/spectrum-poll.error       generated bridge-error dedupe marker, mirroring
                                 state/x-poll.error; suppresses repeat wakes for
                                 the same ensure-bridge failure message
...
1. Calls `bin/fm-spectrum-ensure-bridge.sh` (see "Starting / stopping the
   ...
   stays quiet; an actual restart or start-from-dead is surfaced. An
   ensure-bridge failure is deduped like X mode's relay diagnostics
   (`emit_error_once`/`clear_error` backed by `state/spectrum-poll.error`,
   mirroring `bin/fm-x-poll.sh`'s `state/x-poll.error`): the same failure
   message wakes firstmate once, not every ~20s cycle, and a later distinct
   failure (or a subsequent success, clearing the marker) re-alerts.
